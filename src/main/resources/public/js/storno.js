// Fetch & render /stornoTable into a matrix.
// JSON shape (example):
// {
...
const HEAD = document.getElementById('stornoHead');
const BODY = document.getElementById('stornoBody');
...
function openOrUpdateDetails(pnk, month) {
    const url = `/storno-details.html?pnk=${encodeURIComponent(pnk)}&month=${encodeURIComponent(month)}`;

    // Open (or reuse) a named window "stornoDetails"
    // If it already exists, this navigates it to the new URL.
    detailsWin = window.open(url, "stornoDetails");
...
(async function init() {
    try {
        const data = await fetchJSON('/stornoTable');
        ...
        HEAD.innerHTML = '';
        BODY.innerHTML = `<tr><td>Failed to load /stornoTable</td></tr>`;
